...
function p.getMission(MissionType, MissionTier)
    for i, Miss in pairs(DropData["Missions"]) do
        ...
        if(MissionTier == nil) then
            if(MissionType == Miss.Alias) then
                return Miss
...
local function formatDropString(drop)
    ...
    if(dropType == "Resource") then
        ...
        if(drop.IName == "Mutalist Alad V Nav Coordinate") then
            result = result.."[[Nav Coordinates#Mutalist Nav Coordinates|異融 ALAD V 導航座標]]"
        else
...
function p.getSingleRelicByLocation(tier, name)
    ...
    result = result.."\n!Mission&nbsp;"
    result = result.."\n![[Mission_Rewards#Mission_Tier|Tier]]&nbsp;"
